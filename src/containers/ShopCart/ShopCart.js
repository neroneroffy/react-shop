...
/*                data.goodsItemInfo.map((item)=>{
                ...
                this.setState({
                    ...
                    if(this.state.result){
                        this.state.goodsItemInfo.map((item,index)=>{
                            ...
                            if(item.total>item.specStock){
                                alert(`购物车内 ${item.goodsName} 数量超出商品库存，所选数量自动降至库存数量`)
                                item.total=item.specStock
    ...
    addCount=(index)=>{
        if(this.state.goodsItemInfo[index].total<this.state.goodsItemInfo[index].specStock){
            this.state.goodsItemInfo[index].total = this.state.goodsItemInfo[index].total+1;
            this.setState({});
            this.calculate()
        }else{
    ...
    pay=()=>{
        ...
        this.calculate();
        this.props.transferGoodToOrder(this.state.payInfo);
        let cartPayInfo = [];
        this.state.payInfo.map((item,index)=>{
            cartPayInfo.push(item.goodsId)
        });
        console.log(this.state.payInfo);
        sessionStorage.setItem('goodsId',cartPayInfo.join(','));
        sessionStorage.setItem('goodInfo',JSON.stringify(this.state.payInfo));
        sessionStorage.setItem('lastPage','shopcart')
    };
    goBack=()=>{
        window.history.go(-1)
    };
            ...
            <div className="shop-cart">
                <div className="shop-cart-wrapper">
                    <div className="shop-cart-title clearfix">
                        <Link to={
                            sessionStorage.getItem('lastPage') === 'detail'?'/detail/'+sessionStorage.getItem('goodsId')
                                :
                                sessionStorage.getItem('lastPage') === 'membercenter'?'/membercenter':'/index'
                        } className="go-back">
                            <Icon type="left"/>
                        </Link>
                                        ...
                                        return (
                                            <div className="shop-cart-item" key={index}>
                                                <div className="every-item" >
                                                    {
                                                        item.specStock === 0?
                                                            <span className="del-cart-item" onClick={this.delNoRepertoryItem.bind(this,index)}>
                                                                <Icon type="close" />
                                                            </span>
                                                            :
                                                    ...
                                                    <Link to={'/detail/' + item.goodsId} className="goods-item-in-cart-info">
                ...
                <div className="buy">
                    ...
                    </div>
                    {
                        this.state.totalMoney === 0?
                            ...
                            <Link to="/submitorder" className="pay pay-active" onClick={this.pay.bind(this)}>
...
export default connect(mapPropsToState,mapDispatchToState)(ShopCart);
